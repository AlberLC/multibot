...
class TelegramBot(MultiBot[TelegramClient]):
    ...
    async def clear(self, n_messages: int, chat: int | str | Chat | Message):
        ...
        async with self.user_client:
            ...
            database_messages_to_delete_generator = self._Message.find({'platform': self.platform.value, 'chat': chat.object_id}, sort_keys=(('date', pymongo.DESCENDING),), limit=n_messages, lazy=True)
